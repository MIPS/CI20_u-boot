...
static uint32_t get_mem_clk(void)
{
	uint32_t mpll_out;
	mpll_out =
	    (uint64_t) CONFIG_SYS_EXTAL *JZ4780_MPLL_M / (JZ4780_MPLL_N *
							  JZ4780_MPLL_OD);
...
static void ddr_phy_init(unsigned long ps, unsigned int dtpr0_reg)
{
	...
	unsigned int ptr0_reg = 0, ptr1_reg = 0, ptr2_reg = 0, dtpr1_reg =
	    0, dtpr2_reg = 0;
	...
	/* BL 8 = 0, 0x410 */
	writel(tmp << DDR3_MR0_WR_BIT | (DDR_CL - 4) << 4 | ((8 - DDR_BL) / 2),
	       DDRP_MR0);
#ifdef SDRAM_DISABLE_DLL
	writel(DDR3_MR1_DIC_7 | DDR3_MR1_RTT_DIS | DDR3_MR1_DLL_DISABLE,
	       DDRP_MR1);
#else
	...
	ptr1_reg |=
	    min(DIV_ROUND_UP(DDRP_PTR1_DINIT0_DDR3 * 1000, ps), 0x7ffff);
	...
	ptr2_reg |=
	    min(DIV_ROUND_UP(DDRP_PTR2_DINIT2_DDR3 * 1000, ps), 0x1ffff);
	...
	writel(DDRP_PGCR_DQSCFG |
	       ...
	       DDRP_PGCR_ZCKSEL_32 | DDRP_PGCR_PDDISDX, DDRP_PGCR);

	for (i = 0; i < 8; i++)
		writel(readl(DDRP_DXGCR(i)) & ~(0x3 << 9), DDRP_DXGCR(i));

	while (readl(DDRP_PGSR) !=
	       (DDRP_PGSR_IDONE | DDRP_PGSR_DLDONE | DDRP_PGSR_ZCDONE)) {
	...
	while (readl(DDRP_PGSR) !=
		...
		if (count++ == 500000) {
			if (readl(DDRP_PGSR) &
			    (DDRP_PGSR_DTDONE | DDRP_PGSR_DTERR |
			     DDRP_PGSR_DTIERR))
				hang();
			count = 0;
		}
...
void sdram_init(void)
{
	...
	tmp =
	    (1000000000 % mem_clk ==
	     0) ? (1000000000 / mem_clk) : (1000000000 / mem_clk + 1);
	tmp = DDR_tREFI / tmp;
	...
	if (!mem_size1 && mem_size0 > 0x20000000) {
		mem_base0 = 0x0;
		mem_mask0 =
		    ~(((mem_size0 * 2) >> 24) - 1) & DDRC_MMAP_MASK_MASK;
	} else {
	...
	writel(1 << 15 | 4 << 12 | 1 << 11 | 1 << 8 | 0 << 6 | 1 << 4 | 1 << 3 |
	       1 << 2 | 1 << 1, DDRC_CTRL);
	mem_remap();
