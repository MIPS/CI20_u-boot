...
static int ep_unlink(ohci_t * ohci, ed_t * ed)
{
	...
	case PIPE_CONTROL:
		...
		if (ohci->ed_controltail == ed) {
			...
			((ed_t *)
			 m32_swap(*((__u32 *) & ed->hwNextED)))->ed_prev =
			    ed->ed_prev;
		}
		break;
...
static ed_t *ep_add_ed(struct usb_device *usb_dev, unsigned long pipe)
{
	...
	ed->hwINFO = m32_swap(usb_pipedevice(pipe)
				 ...
				 : (usb_pipeout(pipe) ? 0x800 : 0x1000))
			      | (usb_dev->speed ==
				 USB_SPEED_LOW) << 13 | usb_maxpacket(usb_dev,
								      pipe) <<
			      16);
		    ...
		    struct usb_device *dev, int index, urb_priv_t * urb_priv)
{
	...
	td->data = (__u32) data;
#ifdef OHCI_FILL_TRACE
	if (1 || (usb_pipebulk(urb_priv->pipe) && usb_pipeout(urb_priv->pipe))) {
		for (i = 0; i < len; i++)
			printf("td->data[%d] %#2x\n", i,
			       ((unsigned char *)(td->data + 0x80000000))[i]);
	}
#endif
	if (!len)
		data = 0;
